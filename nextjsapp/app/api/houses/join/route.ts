...
export async function POST(request: NextRequest) {
  ...
  const db = getDb()
  const house = db.prepare('SELECT * FROM houses WHERE invite_code = ?').get(parsed.data.invite_code.toUpperCase()) as
    | { id: number; name: string }
    | undefined

  if (!house) return NextResponse.json({ error: 'Invitationskode ikke fundet' }, { status: 404 })

  const existing = db.prepare('SELECT id FROM house_members WHERE house_id = ? AND user_id = ?').get(house.id, session.userId)
  if (existing) return NextResponse.json({ error: 'Du er allerede medlem af dette hus' }, { status: 409 })

  db.prepare("INSERT INTO house_members (house_id, user_id, role) VALUES (?, ?, 'member')").run(house.id, session.userId)

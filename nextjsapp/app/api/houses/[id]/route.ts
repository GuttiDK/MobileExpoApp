...
async function getMember(db: ReturnType<typeof getDb>, houseId: number, userId: number) {
  return await db.prepare('SELECT role FROM house_members WHERE house_id = ? AND user_id = ?').get(houseId, userId) as
    | { role: string }
    | undefined
}
...
export async function GET(request: NextRequest, { params }: { params: Params }) {
  ...
  const member = await getMember(db, houseId, session.userId)
...
export async function PATCH(request: NextRequest, { params }: { params: Params }) {
  ...
  if (name) await db.prepare('UPDATE houses SET name = ? WHERE id = ?').run(name, houseId)
  if (description !== undefined) await db.prepare('UPDATE houses SET description = ? WHERE id = ?').run(description, houseId)
...
export async function DELETE(request: NextRequest, { params }: { params: Params }) {
  ...
  await db.prepare('DELETE FROM houses WHERE id = ?').run(houseId)
